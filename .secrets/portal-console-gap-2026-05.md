...
1. **CRITICAL, confirmed by test: unauthenticated admin access to butler-server via the Butler proxy.** Surface: butler-backend, butler-server. Effort: ~300 LOC for the full fix (require authenticated Backstage user, drop the email-header trust). Severity reclassified from the original "high / depends on framework default auth policy" after a live test removed the dependency: the framework default policy IS active, and the plugin explicitly opts out of it.
   ...
   **Result. Identical mounting, same backend, only difference is the four-line opt-out:**
   ...
   The 401 from the control proves the framework's default-deny policy is active in this backend, so the 200s under `/api/butler/` are caused by the explicit opt-out, not by an absent policy. The admin Bearer is the real one produced by `AuthManager`'s legacy-admin login flow, so in production it is a genuine butler-server admin session rather than a mock-only artifact.

   **Live deployment evidence.** The production deployment at `https://portal.butlerlabs.dev` (HelmRelease at `butler-portal-live/clusters/butler-portal/apps/butler-portal/helmrelease.yaml`, image tag `20260314023726-sha-e6d04c1`, chart version `0.1.2`) honors the same opt-out. `GET https://portal.butlerlabs.dev/api/butler/_identity` with no auth header returns `{"authenticated":false,"displayName":"Guest","isPlatformAdmin":false,"teams":[]}`, confirming an unauthenticated caller reaches the proxy on the live system. The hostname resolves authoritatively from public resolvers (1.1.1.1 and 8.8.8.8) to `10.40.2.52`, an RFC1918 address, and is reachable across a Tailscale subnet route advertised for `10.40.0.0/16`. The live audience is every tailnet member subject to ACL, not just localhost. The opt-out is therefore an active hole in production, not a launch hole.

   Fix direction (Phase 2 scope, not landed in this audit): remove the `addAuthPolicy({path:'/', allow:'unauthenticated'})` opt-out so the framework default-deny applies, require an authenticated Backstage user on the proxy, adopt the permissions framework for per-action authorization, and stop trusting `X-Butler-User-Email` as identity. The admin/admin default below and the header-identity model are separate follow-on fixes; closing the unauthenticated path is the first cut.
