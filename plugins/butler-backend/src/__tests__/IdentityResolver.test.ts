...
describe('IdentityResolver', () => {
  it('returns undefined for service principals', async () => {
    const r = new IdentityResolver({
      userInfo: userInfoFor('user:default/x'),
      auth,
      logger,
    });
  ...
  it('uses an entity name that already is an email', async () => {
    const r = new IdentityResolver({
      userInfo: userInfoFor('user:default/Dev@Example.com'),
      auth,
      logger,
    });
  ...
  it('prefers the catalog profile email over the domain fallback', async () => {
    const catalog = {
      getEntityByRef: jest.fn(async () => ({
        spec: { profile: { email: 'Dev@Corp.Example' } },
      })),
    } as any;
    const r = new IdentityResolver({
      userInfo: userInfoFor('user:default/dev'),
      auth,
      catalog,
    ...
    expect(catalog.getEntityByRef).toHaveBeenCalledWith('user:default/dev', {
  ...
  it('falls back to the configured domain when the catalog has no entity', async () => {
    ...
    const r = new IdentityResolver({
      userInfo: userInfoFor('user:development/guest'),
      auth,
      catalog,
      emailDomain: 'butlerlabs.dev',
      logger,
    });
    await expect(r.resolveEmail(userCreds)).resolves.toBe(
      'guest@butlerlabs.dev',
    );
  });
  ...
  it('throws when neither the catalog nor a domain can resolve the user', async () => {
    ...
    await expect(r.resolveEmail(userCreds)).rejects.toThrow(
      /cannot resolve an email/,
    );
  });
  ...
  it('remembers an unresolvable user briefly and throws the typed error', async () => {
    ...
    await expect(r.resolveEmail(userCreds)).rejects.toBeInstanceOf(
      UnresolvableIdentityError,
    );
    await expect(r.resolveEmail(userCreds)).rejects.toBeInstanceOf(
      UnresolvableIdentityError,
  ...
  it('does not cache a domain fallback produced by a catalog error', async () => {
    ...
    const catalog = {
      getEntityByRef: jest.fn(async () => {
        ...
        return { spec: { profile: { email: 'real@corp.example' } } };
      }),
    } as any;
    const r = new IdentityResolver({
      userInfo: userInfoFor('user:default/dev'),
  ...
  it('caches the resolved email per entity ref', async () => {
    const catalog = {
      getEntityByRef: jest.fn(async () => ({
        spec: { profile: { email: 'dev@example.com' } },
      })),
    } as any;
    const r = new IdentityResolver({
      userInfo: userInfoFor('user:default/dev'),
  ...
  it('does not remember a catalog outage as an unresolvable user', async () => {
    ...
    const catalog = {
      getEntityByRef: jest.fn(async () => {
        if (fail) throw new Error('down');
        return { spec: { profile: { email: 'dev@corp.example' } } };
      }),
    } as any;
    const r = new IdentityResolver({
      userInfo: userInfoFor('user:default/dev'),
    ...
    await expect(r.resolveEmail(userCreds)).rejects.not.toBeInstanceOf(
      UnresolvableIdentityError,
    );
    fail = false;
    await expect(r.resolveEmail(userCreds)).resolves.toBe('dev@corp.example');
