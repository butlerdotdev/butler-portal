...
const del = page.getByRole('button', { name: /^delete$/i }); if (await del.count() && ROLE !== 'admin') { await del.first().click(); await page.waitForTimeout(800); await shot('delete-dialog'); const confirm = page.getByRole('button', { name: /delete cluster|^delete$/i }).last(); await confirm.click(); await page.waitForTimeout(1500); await shot('delete-result'); log('after delete attempt:', (await text()).match(/forbidden|Failed to delete|permission/gi)?.slice(0,3)); const cancel = page.getByRole('button', { name: /cancel/i }); if (await cancel.count()) await cancel.first().click(); }
