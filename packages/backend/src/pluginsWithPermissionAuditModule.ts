...
// Minimal structural type for the pieces of DynamicPluginProvider the
// audit consumes. Kept as a local shape so tests can inject a plain
// object without importing the framework's full type surface. Generic
// on the plugin shape so tests can hand in plain records while
// production uses the framework's BackendDynamicPlugin.
export interface DynamicPluginsForAudit<TPlugin extends { name: string } = { name: string }> {
  backendPlugins(): TPlugin[];
  getScannedPackage(plugin: TPlugin): {
    manifest: { name: string; backstage?: { pluginId?: string | null } };
  };
}

export interface AuditDeps<TPlugin extends { name: string } = { name: string }> {
  logger: LoggerService;
  config: Config;
  discovery: DiscoveryService;
  auth: AuthService;
  dynamicPlugins: DynamicPluginsForAudit<TPlugin>;
}
...
/**
 * Runs the pluginsWithPermission audit once against the currently-loaded
 * backend plugins. Exposed as a standalone function so tests can inject
 * mocked deps and assert on logger output without booting a full
 * Backstage backend. The module wrapper below calls this from an
 * addStartupHook after a brief settle window.
 */
export async function auditPluginsWithPermission<TPlugin extends { name: string }>(
  deps: AuditDeps<TPlugin>,
): Promise<void> {
  const { logger, config, discovery, auth, dynamicPlugins } = deps;
  ...
  for (const plugin of backendPlugins) {
    ...
    try {
      ...
      // The plugin's own backstage.pluginId in package.json is the
      // authoritative source. Do NOT fall back to manifest.name (the
      // npm package name), because the package name and pluginId are
      // unrelated — e.g. '@backstage-community/plugin-rbac-backend'
      // has pluginId 'permission'. Wrong pluginId leads to a wrong
      // discovery lookup and a bogus 404 the audit would silently
      // accept.
      pluginId = scanned.manifest.backstage?.pluginId ?? undefined;
    } catch (err) {
      logger.warn(
    ...
    try {
      ...
      if (!res.ok) {
        ...
        // addPermissions/addResourceType, so no integration router
        // was wired. Not an error — the plugin is invisible to the
        // audit by design. BUT: if the operator LISTED this plugin
        // in pluginsWithPermission, they expected permissions from
        // it. A 404 there is not benign; it means the plugin did not
        // register what the listing implies. Surface at info so
        // operators auditing logs see the surprise without confusing
        // it for an error.
